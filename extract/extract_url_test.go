	...
	for _, test := range urlTests {
		...
		for n, e := range expected {
			...
			if actual.Text != tmpValue {
				t.Errorf(
					"URLs returned incorrect value for test: [%s]. Expected:[%s] Got:[%s]\n",
					test.Text,
					tmpValue,
					actual.Text,
				)
			...
			if actual.Type != URL {
				t.Errorf(
					"URLs returned entity with wrong type. Expected:URL Got:%v",
					actual.Type,
				)
			}
		}
...
func TestURLsWithIndices(t *testing.T) {
	contents, err := ioutil.ReadFile(extractYmlPath)
	if err != nil {
		t.Errorf("Error reading extract.yml: %v", err)
		t.FailNow()
	...
	for _, test := range urlTests {
		...
		for n, e := range expected {
			...
			if !ok {
				t.Errorf(
					"Expected value did not contain url. Test name: %s\n",
					test.Description,
				)
				continue
			}
			...
			if indicesList[0] != actual.Range.Start || indicesList[1] != actual.Range.Stop {
				t.Errorf(
					"URLs did not return correct indices [%s]. Expected:(%d, %d) Got:%s)",
					test.Text,
					indicesList[0],
					indicesList[1],
					actual.Range,
